...
#       额外计算 reserved 分区之前（0x0 - 0x3810000）的整体 CRC32 值。
...
# 计算 reserved 分区之前的 CRC32（0x0 - 0x3810000）
RESERVED_OFFSET="0x3810000"
reserved_offset_dec=$(hex_to_dec "$RESERVED_OFFSET")
TEMP_IMAGE="temp_reserved.bin"
echo "Generating CRC32 for data before reserved partition (0x0 - $RESERVED_OFFSET)..."
dd if="$OUTPUT_IMAGE" of="$TEMP_IMAGE" bs=1 count="$reserved_offset_dec" status=progress
if [ $? -ne 0 ]; then
    echo "Error: Failed to extract data for reserved CRC!"
    rm -f "$TEMP_IMAGE"
    exit 1
fi
reserved_crc=$(crc32 "$TEMP_IMAGE")
echo "Reserved_Before=$reserved_crc" >> "$CRC_FILE"
echo "  Data before Reserved (0x0 - $RESERVED_OFFSET): CRC32=$reserved_crc"
rm -f "$TEMP_IMAGE"
